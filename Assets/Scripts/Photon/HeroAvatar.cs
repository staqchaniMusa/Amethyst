...
public class HeroAvatar : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        { // send a buffered call to instanciate the players if someone joins the game
            ...
            PlayerInfo.PI.myHeight =0.1f;
            coef = PlayerInfo.PI.myHeight / refHeight;

            // send a buffered call to instanciate the players if someone joins the game
            PV.RPC("RPC_AddCharacter", RpcTarget.AllBuffered, PlayerInfo.PI.NickName, 0.1f);
    ...
    IEnumerator LateAddCharacter(string nickName)
    {
        ...
        if (PV.IsMine)
        {
            Invoke(nameof(EnableCharacter), 0.1f);
        }
